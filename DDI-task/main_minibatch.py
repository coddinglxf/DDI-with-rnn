...
def main():
    ...
    for it in range(1, config.args.it):
        ...
        for i in range(int(batch_num)):
            ...
            # change the learning rate..........., with mini-batch
            if i % batch_size == 0:
                lr_backup = optimizer.param_groups[0]['lr']
                optimizer.param_groups[0]['lr'] = optimizer.param_groups[0]['lr'] / float(batch_size)
                optimizer.step()
                optimizer.param_groups[0]['lr'] = lr_backup
                # print("------------------------------------------------------------------------")
