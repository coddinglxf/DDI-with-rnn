...
def train_test():
    for it in range(config.args.it):
        for batch_time in range(10):
            ...
            # torch.nn.utils.clip_grad_norm(5)
            optimizer.step()
            print("------------------------------------------------------------------------")
...
def main():
    ...
    for it in range(1, config.args.it):
        ...
        for i in range(int(batch_num)):
            ...
            loss.backward()
            clip_grad_norm(seq.parameters(), max_norm=config.args.grad_clip)
            optimizer.step()
            # print("------------------------------------------------------------------------")
        ...
        if it % config.args.eval_interval == 0:
            ...
            if f > 0.69:
                ...
                path = "{0}_{1}_{2}".format(config.args.hidden, it, f)
